...
> **Роль в лекции:** контрпример к человеческому патчу из [`ai_incident_patch.md`](ai_incident_patch.md), используется в блоке 5 сценария ([../script.md](../script.md)) — то, к чему команда приходит по итогам диагностики AI и стабилизации инцидента. Решает ту же задачу (нестабильная станция не должна валить всё задание) через изоляцию, ограниченные повторные попытки и явную деградацию.
